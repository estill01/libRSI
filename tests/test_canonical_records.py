...
def test_identity_is_stable_for_ordering_and_metadata() -> None:
    ...
    changed_claim = Claim(
        statement="Capability does not exist",
        target=first_target,
    )
    assert changed_claim.root != first_claim.root


def test_records_are_deeply_immutable_and_detached_from_inputs() -> None:
    source = {
        "nested": {"items": [1, 2]},
        "enabled": True,
    }
...
def test_exact_references_fail_closed_on_mismatch() -> None:
    ...
    with pytest.raises(TypeError, match="Evidence record"):
        EvidenceRef.from_evidence(claim)  # type: ignore[arg-type]
    with pytest.raises(TypeError, match="evidence references"):
        Outcome(
            intent=goal.ref,
            status="done",
            evidence_refs=(claim.ref,),  # type: ignore[arg-type]
        )


def test_serialization_detects_tampering_and_bad_envelopes() -> None:
    claim = Claim(
        statement="The original claim",
        metadata={"display": "safe to edit"},
    )
    ...
    bad_root = claim.to_dict()
    bad_root["root"] = "broken"
    with pytest.raises(ValueError, match="SHA-256"):
        record_from_dict(bad_root)
...
def test_user_mappings_cannot_collide_with_serialization_markers() -> None:
    payload = {
        "$schema": "librsi.ref/v1",
        "record_type": "not-actually-a-reference",
        "root": "user-data",
        "nested": {
            "$schema": "librsi.record/v1",
            "data": "still user data",
        },
    }
    evidence = Evidence(
    ...
    restored = deserialize_record(serialize_record(evidence))

    assert isinstance(restored, Evidence)
    assert restored.data == FrozenMap(payload)
    assert restored.root == evidence.root


def test_canonical_values_reject_ambiguous_data() -> None:
    with pytest.raises(TypeError, match="string keys"):
        FrozenMap({1: "bad"})  # type: ignore[dict-item]
    with pytest.raises(ValueError, match="finite"):
        Claim(
            statement="bad float",
            scope={"value": float("nan")},
        )
...
def test_record_specific_validation_fails_closed() -> None:
    ...
    with pytest.raises(TypeError, match="TargetRef"):
        TargetSnapshot(
            target=claim,  # type: ignore[arg-type]
            state={},
        )
    with pytest.raises(TypeError, match="components"):
        TargetSnapshot(
            target=target,
            state={},
            components=(claim,),  # type: ignore[arg-type]
        )
    ...
    with pytest.raises(TypeError, match="source references"):
        Hypothesis(
            statement="x",
            source_refs=("bad",),  # type: ignore[arg-type]
        )
    with pytest.raises(TypeError, match="evidence target"):
        Evidence(
            evidence_type="support",
            data={},
            target_snapshot=claim,  # type: ignore[arg-type]
        )
    with pytest.raises(ValueError, match="finite"):
        Evidence(
            evidence_type="support",
    ...
    with pytest.raises(TypeError, match="subject references"):
        Evidence(
            evidence_type="support",
            data={},
            subject_refs=("bad",),  # type: ignore[arg-type]
        )
    ...
    with pytest.raises(TypeError, match="Measurement"):
        Evaluation(
            subject_refs=(claim.ref,),
            disposition="passed",
            measurements=(claim,),  # type: ignore[arg-type]
        )
    ...
    with pytest.raises(TypeError, match="Constraint"):
        Intervention(
            ...
            kind="x",
            specification={},
            constraints=(claim,),  # type: ignore[arg-type]
        )
    with pytest.raises(TypeError, match="Intervention"):
        Candidate(
            intervention=claim.ref,
            target_snapshot=snapshot,
        )
    ...
    with pytest.raises(TypeError, match="ArtifactRef"):
        Candidate(
            intervention=intervention.ref,
            target_snapshot=snapshot,
            artifacts=(claim,),  # type: ignore[arg-type]
        )
    with pytest.raises(TypeError, match="outcome intent"):
        Outcome(
            intent=target,  # type: ignore[arg-type]
            status="done",
        )
