...
_RECORD_TYPES: dict[str, type[SemanticRecord]] = {}
...
def _record_refs(value: Sequence[RecordRef] | None) -> tuple[RecordRef, ...]:
    return tuple(value or ())


def _evidence_refs(value: Sequence[EvidenceRef | RecordRef] | None) -> tuple[EvidenceRef, ...]:
...
class RecordRef:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "record_type", _require_text(self.record_type, "record type"))
    ...
    def to_dict(self) -> dict[str, str]:
        return {"$schema": _REF_SCHEMA, "record_type": self.record_type, "root": self.root}
...
def _decode_value(value: Any) -> Any:
    ...
    if isinstance(value, Mapping):
        ...
        if schema == _REF_SCHEMA:
            ...
            return EvidenceRef(root) if record_type == "evidence" else RecordRef(record_type, root)
...
class SemanticRecord:
    """Base for immutable, self-identifying, durably serializable semantic records."""

    RECORD_TYPE: ClassVar[str] = "record"
    SCHEMA_VERSION: ClassVar[int] = 1

    lineage: tuple[RecordRef, ...] = field(default_factory=tuple)
    metadata: Mapping[str, Any] = field(default_factory=FrozenMap, repr=False)
    root: str = field(init=False)

    def __post_init__(self) -> None:
        object.__setattr__(self, "lineage", _record_refs(self.lineage))
...
class Constraint(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "statement", _require_text(self.statement, "constraint statement"))
        if self.target is not None and not isinstance(self.target, TargetRef):
...
class Hypothesis(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "statement", _require_text(self.statement, "hypothesis statement"))
        if self.target is not None and not isinstance(self.target, TargetRef):
        ...
        object.__setattr__(self, "source_refs", _record_refs(self.source_refs))
...
class Evidence(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "evidence_type", _require_text(self.evidence_type, "evidence type"))
        object.__setattr__(self, "data", _freeze_map(self.data))
        object.__setattr__(self, "subject_refs", _record_refs(self.subject_refs))
        object.__setattr__(self, "source_refs", _record_refs(self.source_refs))
        if self.target_snapshot is not None and not isinstance(self.target_snapshot, TargetSnapshot):
            raise TypeError("evidence target snapshot must be a TargetSnapshot")
        if self.weight is not None:
            object.__setattr__(self, "weight", _finite_number(self.weight, "evidence weight"))
        super().__post_init__()
...
class ExperimentSpec(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "experiment_id", _require_text(self.experiment_id, "experiment id"))
        ...
        object.__setattr__(self, "requested_measurements", _text_tuple(self.requested_measurements))
        super().__post_init__()
...
class Trial(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.experiment, RecordRef) or self.experiment.record_type != "experiment_spec":
        ...
        object.__setattr__(self, "observation_refs", _record_refs(self.observation_refs))
        object.__setattr__(self, "data", _freeze_map(self.data))
        super().__post_init__()
...
class Measurement(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "value", freeze(self.value))
        object.__setattr__(
            self, "unit", None if self.unit is None else _require_text(self.unit, "measurement unit")
        )
        if self.target_snapshot is not None and not isinstance(self.target_snapshot, TargetSnapshot):
...
class Evaluation(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "disposition", _require_text(self.disposition, "evaluation disposition"))
...
class ArtifactRef(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "artifact_id", _require_text(self.artifact_id, "artifact id"))
        ...
        object.__setattr__(
            self,
            "media_type",
            None if self.media_type is None else _require_text(self.media_type, "artifact media type"),
        )
        super().__post_init__()
...
class Candidate(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.intervention, RecordRef) or self.intervention.record_type != "intervention":
...
class Outcome(SemanticRecord):
    ...
    def __post_init__(self) -> None:
        ...
        interventions = _record_refs(self.intervention_refs)
...
def record_from_dict(payload: Mapping[str, Any]) -> SemanticRecord:
    """Reconstruct and integrity-check a canonical record from durable data."""
...
def serialize_record(record: SemanticRecord) -> str:
    """Serialize a canonical record deterministically to UTF-8 JSON text."""
...
def deserialize_record(serialized: str) -> SemanticRecord:
    """Deserialize and integrity-check deterministic canonical record JSON."""
